...
function display_help()
{
    ...
    echo "       --rocshmem-root <path>  Path to rocSHMEM install prefix"
}
...
while true; do
    ...
    -t | --tests_build)              build_tests=true;                                                                                 shift ;;
         ...
         --rocshmem-root)            rocshmem_root=${2};                                                                               shift 2 ;;
    --) shift ; break ;;
...
if [[ "${build_rocshmem_support}" == true ]]; then
    cmake_common_options="${cmake_common_options} -DWITH_ROCSHMEM=ON"
    cmake_common_options="${cmake_common_options} -DROCSHMEM_ROOT=${rocshmem_root}"
fi
